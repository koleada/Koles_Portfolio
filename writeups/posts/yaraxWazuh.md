...
description: "In this post, I show how to integrate the powerful pattern matching and detection tool Yara-X with Wazuh to facilitate automated, customized malware scanning and quarantining on Linux machines."
...
I could go into far greater detail regarding how Yara-X rule creation works; however, the documentation will very likely explain it better than I can. I highly suggest you read [this particular area](https://virustotal.github.io/yara-x/docs/writing_rules/anatomy-of-a-rule/) of the documentation and experiment with detection engineering; it's some cool stuff.

Using these two tools, we can create our own anti-virus scanning that we can tailor to our exact needs, and have the ability to scan for the newest malware before our current anti-virus.
...
Massive shoutouts to [Neo23x0](https://github.com/Neo23x0), [Mr. SOCFortress aka Taylor Walton](https://socfortress.medium.com/), and all contributors to Wazuh, I greatly appreciate all of your time and effort to allow people like me to work with amazing tooling for free. Taylor hugely inspired me to do this, and much of this is based on his video/blog post showcasing the Yara-Wazuh integration.
...
On Windows, the easiest way to install Yara-X is simply by grabbing the latest release from the [https://github.com/VirusTotal/yara-x/releases/tag/v1.14.0](https://github.com/VirusTotal/yara-x/releases/tag/v1.14.0) and simply extracting it.

On Linux, we can install Yara-X by building it ourselves. This does require Rust, so ensure that it is installed prior to running the following:
...
On Mac OS, we can either build it ourselves using the Yara-X repository or install it with brew:
...
This script also compiles all of the rules. Compiling the rules makes it so we only have to compile a single time instead of doing it at runtime each time we want to do a scan.
...
Run the crontab command, then enter what follows at the bottom of the file to run the script every day at 7 pm:
...
We need a way to tell Yara-X what to scan and when to scan it. For this, we use the Wazuh File Integrity Monitoring (FIM) features. We can add directories to our config, specifically within the <syscheck> tags, which will tell Wazuh to monitor those. Our agent then generates events when files within those directories are added, modified, deleted, etc.
...
Real-time means changes get detected as soon as they happen. So as soon as a file in one of those directories is changed, our Wazuh agent sees it and proceeds to perform the "active response" that we will get to shortly.

My thought process was that I wanted Yara-X to scan all binaries I download, which will likely end up in one of the following locations. It can be resource-intensive to do these scans on everything, so at least for my home network, I wanted to go a bit light. Again, the main risk for my home network comes from executables that I download or repositories I clone, so those are what I want to be scanning.

The directories I included are obviously nowhere near exhaustive, but for my needs, they should be more than sufficient.

Importantly, we can also use <ignore> tags to ignore specific paths, effectively excluding them from FIM. This is great for fine-tuning exactly what you want monitored. An example can be seen below:
...
Again, as I mentioned earlier, I wanted to target only executables for my scans. By executables, I really mean files with the +x permission, or files for interpreted languages like Python or bash. Anything that I download that I would consider executing should be scanned by Yara. This is by no means the most comprehensive check for executable files, but again, it fits my current risk profile. One of the benefits of using something like Wazuh is the endless flexibility it has, so I urge you to play around and dial each thing in to your hardware and threat appetite.

So, a quick breakdown of what this script does:

- First, we define our function to check if something is considered an executable to us
...
- Next, we make sure the file size is static before running the scan, which ensures the file is not in the middle of being downloaded, very important
...
Once you have the file, you can confirm it's working properly by running:
...
We need to do a couple of things in the Wazuh Manager itself to enable proper ingestion of our new logs. Specifically, we need to add a decoder and a new rule to our manager.
...
In short, decoders take our raw logs and convert them into a structured format, like key-value pairs. Rules evaluate a log or decoded fields depending on whether the decoder was run or not, and decide if an alert should be generated as a result, and what severity alert to generate.
...
Name is something like yara-rules.xml, and add the following:
...
You should see at least one result there. If you do expand it, you should see fields like full log, with a similar log to the one I showed earlier.

If you do, you successfully configured Yara active response scanning!
